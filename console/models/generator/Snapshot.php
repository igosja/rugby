    ...
    public function execute(): void
    {
        ...
        $sql = "INSERT INTO `snapshot`
                SET `base`=(SELECT AVG(`level`) FROM `team` LEFT JOIN `base` ON `base_id` = `base`.`id` WHERE `team`.`id`!=0),
                    ...
                    `player_position_centre`=(SELECT COUNT(`player`.`id`) FROM `player` LEFT JOIN `player_position` ON `player`.`id`=`player_id` WHERE `team_id`!=0 AND `position_id`=" . Position::CENTRE . ")/(SELECT COUNT(`player`) FROM `player` WHERE `team_id`!=0)*100,
                    `player_position_eight`=(SELECT COUNT(`player`.`id`) FROM `player` LEFT JOIN `player_position` ON `player`.`id`=`player_id` WHERE `team_id`!=0 AND `position_id`=" . Position::EIGHT . ")/(SELECT COUNT(`player`) FROM `player` WHERE `team_id`!=0)*100,
                    `player_position_flanker`=(SELECT COUNT(`player`.`id`) FROM `player` LEFT JOIN `player_position` ON `player`.`id`=`player_id` WHERE `team_id`!=0 AND `position_id`=" . Position::FLANKER . ")/(SELECT COUNT(`player`) FROM `player` WHERE `team_id`!=0)*100,
                    `player_position_fly_half`=(SELECT COUNT(`player`.`id`) FROM `player` LEFT JOIN `player_position` ON `player`.`id`=`player_id` WHERE `team_id`!=0 AND `position_id`=" . Position::FLY_HALF . ")/(SELECT COUNT(`player`) FROM `player` WHERE `team_id`!=0)*100,
                    `player_position_full_back`=(SELECT COUNT(`player`.`id`) FROM `player` LEFT JOIN `player_position` ON `player`.`id`=`player_id` WHERE `team_id`!=0 AND `position_id`=" . Position::FULL_BACK . ")/(SELECT COUNT(`player`) FROM `player` WHERE `team_id`!=0)*100,
                    `player_position_hooker`=(SELECT COUNT(`player`.`id`) FROM `player` LEFT JOIN `player_position` ON `player`.`id`=`player_id` WHERE `team_id`!=0 AND `position_id`=" . Position::HOOKER . ")/(SELECT COUNT(`player`) FROM `player` WHERE `team_id`!=0)*100,
                    `player_position_lock`=(SELECT COUNT(`player`.`id`) FROM `player` LEFT JOIN `player_position` ON `player`.`id`=`player_id` WHERE `team_id`!=0 AND `position_id`=" . Position::LOCK . ")/(SELECT COUNT(`player`) FROM `player` WHERE `team_id`!=0)*100,
                    `player_position_prop`=(SELECT COUNT(`player`.`id`) FROM `player` LEFT JOIN `player_position` ON `player`.`id`=`player_id` WHERE `team_id`!=0 AND `position_id`=" . Position::PROP . ")/(SELECT COUNT(`player`) FROM `player` WHERE `team_id`!=0)*100,
                    `player_position_scrum_half`=(SELECT COUNT(`player`.`id`) FROM `player` LEFT JOIN `player_position` ON `player`.`id`=`player_id` WHERE `team_id`!=0 AND `position_id`=" . Position::SCRUM_HALF . ")/(SELECT COUNT(`player`) FROM `player` WHERE `team_id`!=0)*100,
                    `player_position_wing`=(SELECT COUNT(`player`.`id`) FROM `player` LEFT JOIN `player_position` ON `player`.`id`=`player_id` WHERE `team_id`!=0 AND `position_id`=" . Position::WING . ")/(SELECT COUNT(`player`) FROM `player` WHERE `team_id`!=0)*100,
